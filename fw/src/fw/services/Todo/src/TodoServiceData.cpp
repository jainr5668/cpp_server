    ...
    namespace TodoService
    {
        TodoServiceData::TodoServiceData(std::shared_ptr<IData> data)
        {
            ...
            std::vector<std::string> queries = {
                "CREATE TABLE IF NOT EXISTS todos (id TEXT, userId TEXT NOT NULL, title VARCHAR(255) NOT NULL, description TEXT NOT NULL, "
                "status VARCHAR(100), dueDate DATETIME, created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL);"
            };
        ...
        bool TodoServiceData::updateTodoById(TodoDbData todo){
            ...
            std::string query = "UPDATE todos SET title = '" + todo.title.value + "', description = '" + todo.description.value + "', status = '" +\
                todo.status.value + "', dueDate = '" + Utils::timepoint_to_string(todo.dueDate.value) + "', updated_at = '" + \
                Utils::timepoint_to_string(todo.updated_at.value) + "' WHERE id = '" + todo.id.value() + "';";
            data_->execute(query);
